...
class SpendingController {
    ...
    async listSpendingsInBudget(req, res) {
        ...
        try {
            const spendings = await SpendingService.listSpendingsInBudget(budgetId, description, startDate ? new Date(startDate) : new Date(), endDate ? new Date(endDate) : new Date());
            res.json(spendings);
        } catch (error) {
            res.status(StatusCode.statusCodeFromErrorType(error)).json({ error: error.message });
        }
